...
class GLM:
    def __init__(self, family='gaussian', learning_rate=0.01, epochs=1000, tol=1e-6, reg_lambda=0.0):
        self.family = family
    ...
    def _loss(self, y_true, y_pred):
        if self.family == 'gaussian':
            ...
            l2_penalty = self.reg_lambda * cp.sum(self.weights[1:] ** 2)  # exclude bias term
            return mse + l2_penalty
        elif self.family == 'binomial':
            eps = 1e-10
            y_pred = cp.clip(y_pred, eps, 1 - eps)
            log_loss = -cp.mean(y_true * cp.log(y_pred) + (1 - y_true) * cp.log(1 - y_pred))
            l2_penalty = self.reg_lambda * cp.sum(self.weights[1:] ** 2)
    ...
    def _gradient(self, X, y_true, y_pred):
        grad = cp.dot(X.T, (y_pred - y_true)) / X.shape[0]
        # Do not regularize the bias term (weights[0])
        grad[1:] += 2 * self.reg_lambda * self.weights[1:]
        return grad
...
def train_model(data, family='binomial'):
    ...
    for lr in learning_rates:
        for ep in epochs_list:
            model = GLM(family=family, learning_rate=lr, epochs=ep, reg_lambda=0.01)
...
if __name__ == '__main__':
    ...
    start_regression(model_file, 'Iris.csv', 'Species', family='gaussian')
